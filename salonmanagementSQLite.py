...
def _get_base_dir():
    """Directory next to the .exe (or the script when running from source)."""
    if getattr(sys, 'frozen', False):
        return os.path.dirname(sys.executable)
    return os.path.dirname(os.path.abspath(__file__))

_BASE_DIR    = _get_base_dir()
_CONFIG_FILE = os.path.join(_BASE_DIR, "salon_config.json")
...
def _save_config(db_path, icon_dir):
    with open(_CONFIG_FILE, 'w') as f:
        json.dump({"db_path": db_path, "icon_dir": icon_dir}, f, indent=2)

def _show_path_dialog(current_db="", current_icons=""):
    """
    Plain-tkinter dialog to let the user choose the database file and icons folder.
    Returns (db_path, icon_dir) on confirm, or (None, None) on cancel.
    """
    import tkinter as tk
    from tkinter import filedialog as fd, messagebox

    result = {"db": None, "icons": None, "ok": False}

    root = tk.Tk()
    root.title("Salon Manager – Configure Paths")
    root.resizable(False, False)
    root.configure(bg="#1a1a1a")
    root.geometry("580x230")
    root.update_idletasks()
    x = (root.winfo_screenwidth()  - 580) // 2
    y = (root.winfo_screenheight() - 230) // 2
    root.geometry(f"580x230+{x}+{y}")

    FG, BG, EBG, GOLD = "#f0f0f0", "#1a1a1a", "#2c2c2c", "#c9a84c"
    FONT = ("Segoe UI", 10)

    tk.Label(root, text="Salon Manager – Configure Paths",
             bg=BG, fg=GOLD, font=("Segoe UI", 13, "bold")).grid(
             row=0, column=0, columnspan=3, padx=20, pady=(18, 14), sticky="w")

    # ── Database file ──
    tk.Label(root, text="Database file (.db):", bg=BG, fg=FG, font=FONT).grid(
        row=1, column=0, padx=(20, 8), pady=4, sticky="w")
    default_db = current_db or os.path.join(_BASE_DIR, "salonmanagement.db")
    db_var = tk.StringVar(value=default_db)
    tk.Entry(root, textvariable=db_var, width=44, bg=EBG, fg=FG,
             insertbackground=FG, relief="flat", font=FONT).grid(
             row=1, column=1, pady=4, sticky="ew")
    tk.Button(root, text="Browse…", bg="#333", fg=FG, relief="flat", font=FONT,
              command=lambda: db_var.set(
                  fd.asksaveasfilename(
                      defaultextension=".db",
                      filetypes=[("Database", "*.db"), ("All files", "*.*")],
                      initialfile=os.path.basename(db_var.get()),
                      initialdir=os.path.dirname(db_var.get()) or _BASE_DIR)
                  or db_var.get())).grid(row=1, column=2, padx=(8, 20), pady=4)

    # ── Icons folder ──
    tk.Label(root, text="Icons folder:", bg=BG, fg=FG, font=FONT).grid(
        row=2, column=0, padx=(20, 8), pady=4, sticky="w")
    icon_var = tk.StringVar(value=current_icons)
    tk.Entry(root, textvariable=icon_var, width=44, bg=EBG, fg=FG,
             insertbackground=FG, relief="flat", font=FONT).grid(
             row=2, column=1, pady=4, sticky="ew")
    tk.Button(root, text="Browse…", bg="#333", fg=FG, relief="flat", font=FONT,
              command=lambda: icon_var.set(
                  fd.askdirectory(initialdir=icon_var.get() or _BASE_DIR)
                  or icon_var.get())).grid(row=2, column=2, padx=(8, 20), pady=4)

    root.grid_columnconfigure(1, weight=1)

    def _ok():
        db    = db_var.get().strip()
        icons = icon_var.get().strip()
        if not db:
            messagebox.showerror("Error", "Please set a database file path.", parent=root)
            return
        if not icons:
            messagebox.showerror("Error", "Please set an icons folder path.", parent=root)
            return
        result.update({"db": db, "icons": icons, "ok": True})
        root.destroy()
    ...
    bf.grid(row=3, column=0, columnspan=3, pady=(18, 10))
    tk.Button(bf, text="Save & Continue", bg=GOLD, fg="#1a1a1a",
              font=("Segoe UI", 10, "bold"), relief="flat", padx=16,
              command=_ok).pack(side="left", padx=8)
    tk.Button(bf, text="Cancel", bg="#333", fg=FG, font=FONT,
              relief="flat", padx=12, command=_cancel).pack(side="left", padx=8)

    root.mainloop()
    return (result["db"], result["icons"]) if result["ok"] else (None, None)


# ── Boot: load or request configuration ──────────────────────────
_cfg = _load_config()
if _cfg is None:
    _db, _icons = _show_path_dialog()
    if _db is None:
        sys.exit(0)
    _save_config(_db, _icons)
    _cfg = {"db_path": _db, "icon_dir": _icons}

# ═══════════════════════════════════════════════════════════════
#  VALIDATION HELPERS
# ═══════════════════════════════════════════════════════════════

def _vld_email(email: str) -> bool:
    """Empty is allowed (optional field). If provided must look like x@y.z"""
    if not email: return True
    return bool(re.match(r'^[^@\s]+@[^@\s]+\.[^@\s]+$', email))

def _vld_phone(phone: str) -> bool:
    """Empty is allowed (optional field). If provided must be 7–8 digits only."""
    if not phone: return True
    return bool(re.match(r'^\d{7,8}$', phone))

def _vld_date(date_str: str):
    """Returns (True, date_obj) on success or (False, None) on failure."""
    try:
        return True, datetime.strptime(date_str.strip(), "%Y-%m-%d").date()
    except ValueError:
        return False, None

def _vld_pos_float(value_str: str):
    """Returns (True, float) if value is a positive number, else (False, None)."""
    try:
...
def _vld_nonneg_int(value_str: str):
    """Returns (True, int) if value is a non-negative integer, else (False, None)."""
    try:
...
def _show_errors(err_widget, messages: list) -> bool:
    """
    Filter out empty strings, join the remainder with bullet points,
    display on err_widget. Returns True if there were any errors.
    """
    msgs = [m for m in messages if m]
    if not msgs:
        return False
    bullet = "  •  "
    err_widget.configure(text=bullet + f"\n{bullet}".join(msgs))
    return True
# ═══════════════════════════════════════════════════════════════
#  DATABASE
# ═══════════════════════════════════════════════════════════════
...
# SQLite compatibility layer for the original Microsoft Access-style queries.
def _sqlite_parse_date(value):
    if value is None:
        return None
    ...
    if not text:
        return None
    for fmt in ("%Y-%m-%d", "%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S", "%d/%m/%Y", "%H:%M"):
        try:
            return datetime.strptime(text[:len(datetime.now().strftime(fmt))] if fmt != "%H:%M" else text, fmt)
        except Exception:
            pass
    try:
...
def _sqlite_datevalue(value):
    dt = _sqlite_parse_date(value)
...
def _sqlite_weekday(value, first_day=1):
    dt = _sqlite_parse_date(value)
    if not dt:
        return None
    # Access Weekday(date, 2): Monday=1 ... Sunday=7
    if int(first_day or 1) == 2:
        return dt.weekday() + 1
    # Access default: Sunday=1 ... Saturday=7
    return ((dt.weekday() + 1) % 7) + 1

def _sqlite_format(value, pattern):
    dt = _sqlite_parse_date(value)
    if not dt:
        return ""
    ...
    if pat in ("yyyy-mm-dd", "short date"):
        return dt.strftime("%Y-%m-%d")
    return dt.strftime("%Y-%m-%d")

def _sqlite_iif(condition, yes_value, no_value):
    return yes_value if condition else no_value

def _sqlite_cstr(value):
...
def _normalize_params(params):
    ...
    for v in params:
        if hasattr(v, "strftime"):
            out.append(v.strftime("%Y-%m-%d %H:%M" if hasattr(v, "hour") else "%Y-%m-%d"))
        else:
            out.append(v)
    return tuple(out)

def _access_sql_to_sqlite(sql):
    # SQLite accepts [bracketed identifiers], ? parameters, TRUE/FALSE, and most JOINs.
    text = sql
    # Access TOP n -> SQLite LIMIT n
    import re as _re
    m = _re.search(r"\bSELECT\s+TOP\s+(\d+)\s+", text, flags=_re.I)
    if m:
        limit = m.group(1)
        text = _re.sub(r"\bSELECT\s+TOP\s+\d+\s+", "SELECT ", text, count=1, flags=_re.I)
        if not _re.search(r"\bLIMIT\b", text, flags=_re.I):
            text = text.rstrip().rstrip(";") + f" LIMIT {limit}"
    return text
...
class SQLiteCompatCursor:
    ...
    def execute(self, sql, params=()):
        sql = _access_sql_to_sqlite(sql)
        return self.inner.execute(sql, _normalize_params(params))
    def executemany(self, sql, seq):
        sql = _access_sql_to_sqlite(sql)
        return self.inner.executemany(sql, [_normalize_params(p) for p in seq])
    def fetchone(self):
...
def init_database(conn):
    conn.create_function("DateValue", 1, _sqlite_datevalue)
    conn.create_function("Month", 1, _sqlite_month)
    conn.create_function("Year", 1, _sqlite_year)
    conn.create_function("Weekday", 1, lambda v: _sqlite_weekday(v, 1))
    conn.create_function("Weekday", 2, _sqlite_weekday)
    conn.create_function("Format", 2, _sqlite_format)
    conn.create_function("IIf", 3, _sqlite_iif)
    conn.create_function("CStr", 1, _sqlite_cstr)
    ...
    CREATE TABLE IF NOT EXISTS Clients (
        ...
        Email TEXT,
        PhoneNumber TEXT,
        DateJoined TEXT DEFAULT (date('now')),
        ProfilePicture TEXT
    );

    CREATE TABLE IF NOT EXISTS Hairdressers (
        HairdresserID INTEGER PRIMARY KEY AUTOINCREMENT,
        HairdresserName TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS SlotData (
        ID INTEGER PRIMARY KEY AUTOINCREMENT,
        Timings TEXT NOT NULL UNIQUE
    );

    CREATE TABLE IF NOT EXISTS Bookings (
        BookingID INTEGER PRIMARY KEY AUTOINCREMENT,
        ClientID INTEGER,
        HairdresserID INTEGER,
        ServiceID INTEGER,
        [Date] TEXT NOT NULL,
        Slot INTEGER,
        ...
        FOREIGN KEY(HairdresserID) REFERENCES Hairdressers(HairdresserID),
        FOREIGN KEY(ServiceID) REFERENCES Services(ServiceID),
        FOREIGN KEY(Slot) REFERENCES SlotData(ID)
    );
    ...
    CREATE TABLE IF NOT EXISTS OrderItems (
        ...
        PriceAtSale REAL NOT NULL DEFAULT 0,
    ...
    # Default time slots so bookings work immediately on a fresh database.
    default_slots = [
        "09:00", "09:30", "10:00", "10:30", "11:00", "11:30",
        "12:00", "12:30", "13:00", "13:30", "14:00", "14:30",
        "15:00", "15:30", "16:00", "16:30", "17:00"
    ]
    c.executemany("INSERT OR IGNORE INTO SlotData(Timings) VALUES(?)", [(x,) for x in default_slots])
    conn.commit()

init_database(conn)
cursor = SQLiteCompatCursor(conn.cursor())
...
ICON_PATHS = {
    ...
    "services": os.path.join(ICON_DIR, "toiletries.png"),  # reuse products icon
    "reports": os.path.join(ICON_DIR, "orders.png"),        # reuse orders icon
...
def format_date(v, fmt="%Y-%m-%d"):
    ...
    dt = _sqlite_parse_date(v)
    return dt.strftime(fmt) if dt else str(v)
...
def format_slot(v):
    ...
    text = str(v)
    dt = _sqlite_parse_date(text)
    return dt.strftime("%I:%M %p").lstrip("0") if dt else text
...
def db_busiest_day():
    ...
    cursor.execute("""
        ...
        WHERE date([Date])>=? AND date([Date])<? AND Canceled=False
...
def db_upcoming_bookings():
    cursor.execute(_BOOKING_SELECT +
        "WHERE Bookings.Canceled=False AND date(Bookings.[Date])>=date('now') AND date(Bookings.[Date])<=date('now','+1 day') "
...
def db_booking_refs():
    ...
    for r in slot_rows:
        raw = r[1]
        label = raw.strftime('%H:%M') if hasattr(raw, 'strftime') else str(raw)
        slot_map[label] = r[0]
...
_sidebar_btn("Services",      "services",     "Services")
_sidebar_btn("Schedule",      "schedule",     "Schedule")
_sidebar_btn("Orders",        "orders",       "Orders")
_sidebar_btn("Financials",    "finance",      "Financials")
_sidebar_btn("Reports",       "reports",      "Reports")

# ── Change Paths button ───────────────────────────────────────
CTkFrame(sidebar, fg_color=C_BORDER, height=1).pack(fill="x", padx=16, pady=(12, 8))

def _open_path_settings():
    from tkinter import messagebox
    cur = _load_config() or {}
    new_db, new_icons = _show_path_dialog(
        current_db    = cur.get("db_path", DB_PATH),
        current_icons = cur.get("icon_dir", ICON_DIR)
    )
    if new_db is None:
        return  # user cancelled
    _save_config(new_db, new_icons)
    messagebox.showinfo(
        "Paths Saved",
        "Paths have been saved.\n\nPlease restart Salon Manager for the changes to take effect."
    )

CTkButton(
    sidebar, text="⚙  Change Paths", anchor="w",
    width=198, height=36, corner_radius=10,
    fg_color="transparent", hover_color=C_SURFACE,
    text_color=C_TEXT_DIM, font=FONT_BODY,
    command=_open_path_settings
...
services_frame          = _make_tab()
hairdresser_bookings_frame = _make_tab()
orders_frame            = _make_tab()
financials_frame        = _make_tab()
reports_frame           = _make_tab()

ALL_FRAMES = [dashboard_frame, hairdresser_frame, clients_frame,
              products_frame, services_frame, hairdresser_bookings_frame,
              orders_frame, financials_frame, reports_frame]
...
def edit_booking_popup(booking_id):
    ...
    def save():
        ...
        dv = de.get().strip()
        if not dv:
            errs.append("Date is required — enter it in YYYY-MM-DD format (e.g. 2025-12-31).")
        else:
            ok, parsed_date = _vld_date(dv)
            if not ok:
                errs.append(f"'{dv}' is not a valid date. Use YYYY-MM-DD (e.g. 2025-06-15).")
            elif parsed_date < datetime.now().date():
                errs.append("Bookings cannot be set to a past date. Please choose today or a future date.")

        # ── Dropdown selections ──────────────────────────────────
        if cb_c.get() not in cm:
            errs.append("Please select a valid client from the dropdown.")
        if cb_h.get() not in hm:
            errs.append("Please select a valid hairdresser from the dropdown.")
        if cb_s.get() not in sm:
            errs.append("Please select a valid service from the dropdown.")
        if cb_sl.get() not in slm:
            errs.append("Please select a valid time slot from the dropdown.")

        if _show_errors(err, errs):
            return
        ...
        try:
            cursor.execute(
                ...
                "WHERE HairdresserID=? AND Slot=? AND [Date]=? AND BookingID<>? AND Canceled=False",
                (hid, slid, dv, booking_id)
            )
            if cursor.fetchone()[0] > 0:
                hairdresser_name = cb_h.get()
                slot_label       = cb_sl.get()
                err.configure(
                    text=f"Double-booking prevented:\n"
                         f"{hairdresser_name} already has a booking on {dv} at {slot_label}.\n"
                         f"Please choose a different hairdresser, date, or time slot."
                )
                return
        except Exception as db_err:
            err.configure(text="Could not check for conflicts. Please try again.")
            return
        ...
        try:
            ...
            err.configure(text="The booking could not be saved. Please check all fields and try again.")
    ...
    def save():
        ...
        if not dv:
            ...
            if not ok:
                ...
                errs.append("Bookings cannot be made for past dates. Please select today or a future date.")

        # ── Dropdowns ───────────────────────────────────────────
        if not cm:
            errs.append("No clients exist. Add a client before creating a booking.")
        elif cb_c.get() not in cm:
            errs.append("Please select a valid client from the dropdown.")

        if not hm:
            errs.append("No hairdressers exist. Add a hairdresser before creating a booking.")
        elif cb_h.get() not in hm:
            errs.append("Please select a valid hairdresser from the dropdown.")

        if not sm:
            errs.append("No services exist. Add a service before creating a booking.")
        elif cb_s.get() not in sm:
            errs.append("Please select a valid service from the dropdown.")

        if not slm:
            errs.append("No time slots are configured in the database.")
        elif cb_sl.get() not in slm:
            errs.append("Please select a valid time slot from the dropdown.")
        ...
        try:
            cursor.execute(
                "SELECT COUNT(*) FROM Bookings "
                "WHERE HairdresserID=? AND Slot=? AND [Date]=? AND Canceled=False",
                (hid, slid, dv)
            )
            if cursor.fetchone()[0] > 0:
                hairdresser_name = cb_h.get()
            ...
            err.configure(text="Could not check for scheduling conflicts. Please try again.")
            return

        # ── Save ─────────────────────────────────────────────────
        try:
            cursor.execute(
                "INSERT INTO Bookings(ClientID,HairdresserID,ServiceID,[Date],Slot,IsCompleted,Canceled) "
                "VALUES(?,?,?,?,?,?,False)",
                (cid, hid, sid, dv, slid, sw.get())
            )
            conn.commit()
            overlay.destroy()
            refresh_dashboard()
...
def edit_hairdresser_popup(hid):
    ...
    h = cursor.fetchone()
    if not h:
        err.configure(text="Hairdresser record not found. It may have been deleted."); return
    ...
    def save():
        errs = []
        n       = ne.get().strip()
        exp_str = ee.get().strip()
        if not n:
            errs.append("Hairdresser name is required.")
        if not exp_str:
            errs.append("Years of experience is required.")
        else:
            try:
                exp_val = int(exp_str)
                if exp_val < 0:
                    errs.append("Years of experience cannot be negative — enter 0 or more.")
            except ValueError:
                errs.append(f"'{exp_str}' is not valid for years of experience. Enter a whole number (e.g. 3).")
                exp_val = None
        if _show_errors(err, errs): return
        exp_val = int(exp_str)
        try:
            cursor.execute(
                "UPDATE Hairdressers SET HairdresserName=?,YearsOfExperience=?,[Additional Notes]=?,ProfilePicture=? WHERE HairdresserID=?",
                (n, exp_val, no.get().strip(), pe.get().strip(), hid)
            )
            conn.commit(); overlay.destroy(); refresh_hairdresser_page()
        except Exception:
            err.configure(text="The hairdresser record could not be saved. Please try again.")

    def delete():
        try:
            cursor.execute("DELETE FROM Hairdressers WHERE HairdresserID=?", (hid,))
            conn.commit()
        except Exception:
            err.configure(text="This hairdresser cannot be deleted — they may still have bookings linked to their record.")
            return
...
def add_hairdresser_popup():
    ...
    def save():
        ...
        try:
            cursor.execute(
                "INSERT INTO Hairdressers(HairdresserName,YearsOfExperience,[Additional Notes],ProfilePicture) VALUES(?,?,?,?)",
                (n, exp_val, no.get().strip(), pe.get().strip())
            )
            conn.commit(); overlay.destroy(); refresh_hairdresser_page()
        except Exception:
            err.configure(text="The hairdresser could not be added. Please try again.")
...
def edit_client_popup(cid):
    ...
    cl = cursor.fetchone()
    if not cl:
        err.configure(text="Client record not found. It may have been deleted."); return
    ...
    def save():
        ...
        if email and not _vld_email(email):
            errs.append(f"'{email}' is not a valid email address. It should look like name@example.com.")
        if phone and not _vld_phone(phone):
            errs.append(f"'{phone}' is not a valid phone number. Enter 7 or 8 digits only (no spaces or dashes).")
        if _show_errors(err, errs): return
        try:
            cursor.execute(
                "UPDATE Clients SET ClientName=?,Email=?,PhoneNumber=?,ProfilePicture=? WHERE ClientID=?",
                (n, email, phone, pie.get().strip(), cid)
            )
            conn.commit(); overlay.destroy(); refresh_clients_page()
        except Exception:
            err.configure(text="The client record could not be saved. Please try again.")

    def delete():
        try:
            cursor.execute("DELETE FROM Clients WHERE ClientID=?", (cid,))
            conn.commit()
        except Exception:
            err.configure(text="This client cannot be deleted — they may still have bookings or orders linked to their record.")
            return
...
def add_client_popup():
    ...
    def save():
        ...
        try:
            cursor.execute(
                "INSERT INTO Clients(ClientName,Email,PhoneNumber,ProfilePicture,DateJoined) VALUES(?,?,?,?,?)",
                (n, email, phone, pie.get().strip(), datetime.now().strftime("%Y-%m-%d"))
            )
            conn.commit(); overlay.destroy(); refresh_clients_page()
        except Exception:
            err.configure(text="The client could not be added. Please try again.")
...
def edit_product_popup(pid):
    ...
    try:
        ...
        err.configure(text="Could not load the product. Please try again."); return
    if not p:
        err.configure(text="Product not found. It may have been deleted."); return
    ...
    def save():
        errs = []
        n       = ne.get().strip()
        prc_str = pre.get().strip()
        stk_str = se.get().strip()
        if not n:
            errs.append("Product name is required.")
        if not prc_str:
            errs.append("Price is required.")
        else:
            ok, prc_val = _vld_pos_float(prc_str)
            if not ok:
                errs.append(f"'{prc_str}' is not a valid price. Enter a positive number (e.g. 250.00).")
        if not stk_str:
            errs.append("Stock quantity is required.")
        else:
            ok2, stk_val = _vld_nonneg_int(stk_str)
            if not ok2:
                errs.append(f"'{stk_str}' is not valid for stock. Enter a whole number of 0 or more (e.g. 10).")
        if _show_errors(err, errs): return
        prc_val = float(prc_str)
        stk_val = int(stk_str)
        try:
            cursor.execute(
                "UPDATE Products SET ProductName=?,Price=?,ProductDescription=?,StockRemaining=?,ProductImage=? WHERE ProductID=?",
                (n, prc_val, de.get().strip(), stk_val, ie.get().strip(), pid)
            )
            conn.commit(); overlay.destroy(); refresh_products_page()
        except Exception:
            err.configure(text="The product could not be saved. Please try again.")
    ...
    def delete():
        try:
            ...
            err.configure(text="This product cannot be deleted — it may still be referenced in existing orders.")
            return
...
def add_product_popup():
    ...
    def save():
        ...
        try:
            cursor.execute(
                "INSERT INTO Products(ProductName,Price,ProductDescription,StockRemaining,ProductImage) VALUES(?,?,?,?,?)",
                (n, prc_val, de.get().strip(), stk_val, ie.get().strip())
            )
            conn.commit(); overlay.destroy(); refresh_products_page()
        except Exception:
            err.configure(text="The product could not be added. Please try again.")
...
def db_services():
    try:
        cursor.execute("SELECT ServiceID, ServiceName, Price, ServiceDescription FROM Services ORDER BY ServiceName")
        return cursor.fetchall()
    except Exception as e:
        print("DB SERVICES:", e); return []

def refresh_services_page():
    clear_frame(services_frame)
    _build_services(services_frame)

def _build_services(parent):
    top = CTkFrame(parent, fg_color="transparent")
    top.pack(fill="x", padx=PAD, pady=(PAD, 8))
    CTkLabel(top, text="Services", font=FONT_TITLE, text_color=C_TEXT).pack(side="left")
    ctrl = CTkFrame(top, fg_color="transparent"); ctrl.pack(side="right")
    _gold_btn(ctrl, "+ Add Service", add_service_popup, width=130).pack(side="left")
    ...
    if not data:
        CTkLabel(scroll, text="No services yet. Add your first service.",
                 font=FONT_BODY, text_color=C_MUTED).pack(pady=40)
        return

    CTkLabel(scroll, text="All Services", font=FONT_SECTION,
             text_color=C_GOLD).pack(anchor="w", pady=(12, 6))
    grid = CTkFrame(scroll, fg_color="transparent")
    grid.pack(fill="x")
    cols = 3
    for c in range(cols):
        grid.grid_columnconfigure(c, weight=1, uniform="sv")
    for idx, s in enumerate(data):
        sid, name, price, desc = s
        r, c = divmod(idx, cols)
        card = CTkFrame(grid, fg_color=C_SURFACE, corner_radius=RADIUS)
        card.grid(row=r, column=c, padx=10, pady=10, sticky="nsew")
        _service_card_content(card, sid, name, price, desc)

def _service_card_content(card, sid, name, price, desc):
    clear_frame(card)
    CTkLabel(card, text="✂", font=("Inter", 44), text_color=C_GOLD).pack(pady=(20, 6))
    CTkLabel(card, text=name, font=FONT_BOLD, text_color=C_TEXT,
             wraplength=230).pack(padx=12)
    sp = safe_float(price)
    CTkLabel(card,
             text=f"Rs {sp:,.2f}" if sp > 0 else "Price not set",
             font=("Inter", 14, "bold"), text_color=C_GOLD).pack(pady=(4, 6))
    CTkLabel(card, text=desc or "No description", font=FONT_SMALL,
             text_color=C_MUTED, wraplength=230, justify="center").pack(pady=(0, 14))
    CTkFrame(card, fg_color=C_BORDER, height=1).pack(fill="x", padx=16)
    _ghost_btn(card, "Edit", lambda id=sid: edit_service_popup(id),
               width=80, height=30).pack(pady=10)

def _service_form(panel):
    ne  = labeled_entry(panel, "Service Name",    1, 0, width=280)
    pre = labeled_entry(panel, "Price (Rs)",       1, 1, width=180)
    de  = labeled_entry(panel, "Description",      3, 0, width=500)
    return ne, pre, de

def edit_service_popup(sid):
    overlay, panel, err = overlay_modal(app, "Edit Service", width=680, height=400)
    try:
        cursor.execute(
            "SELECT ServiceID, ServiceName, Price, ServiceDescription FROM Services WHERE ServiceID=?",
            (sid,))
        s = cursor.fetchone()
    except Exception:
        err.configure(text="Could not load the service record. Please try again."); return
    if not s:
        err.configure(text="Service not found. It may have been deleted."); return
    _, name, price, desc = s
    ne, pre, de = _service_form(panel)
    ne.insert(0, name or "")
    pre.insert(0, str(price or ""))
    de.insert(0, desc or "")
    ...
    def save():
        ...
        prc_str = pre.get().strip()
        if not n:
            errs.append("Service name is required.")
        if not prc_str:
            errs.append("Price is required.")
        else:
            ok, prc_val = _vld_pos_float(prc_str)
            if not ok:
                errs.append(f"'{prc_str}' is not a valid price. Enter a positive number in Rs (e.g. 500.00).")
        if _show_errors(err, errs): return
        prc_val = float(prc_str)
        try:
            cursor.execute(
                "UPDATE Services SET ServiceName=?, Price=?, ServiceDescription=? WHERE ServiceID=?",
                (n, prc_val, de.get().strip(), sid))
            conn.commit(); overlay.destroy(); refresh_services_page()
        except Exception:
            err.configure(text="The service could not be saved. Please try again.")

    def delete():
        try:
            cursor.execute("DELETE FROM Services WHERE ServiceID=?", (sid,))
            conn.commit()
        except Exception:
            err.configure(text="This service cannot be deleted — it may still be linked to existing bookings.")
            return
        overlay.destroy(); refresh_services_page()

    modal_buttons(panel, overlay, save, delete, "Save Changes", "Delete Service")

def add_service_popup():
    overlay, panel, err = overlay_modal(app, "Add Service", width=680, height=400)
    ne, pre, de = _service_form(panel)
    ...
    def save():
        ...
        try:
            cursor.execute(
                "INSERT INTO Services(ServiceName, Price, ServiceDescription) VALUES(?, ?, ?)",
                (n, prc_val, de.get().strip()))
            conn.commit(); overlay.destroy(); refresh_services_page()
        except Exception:
            err.configure(text="The service could not be added. Please try again.")

    modal_buttons(panel, overlay, save, save_text="Create Service")
    ...
    def load(mode):
        ...
        if mode=="upcoming":
            cursor.execute(_BOOKING_SELECT.replace("INNER JOIN SlotData","INNER JOIN SlotData").replace("SELECT Bookings.BookingID,","SELECT Bookings.BookingID,").replace(
                "INNER JOIN SlotData    ON Bookings.Slot=SlotData.ID",
                "INNER JOIN SlotData    ON Bookings.Slot=SlotData.ID"
            ) + "WHERE Bookings.Canceled=False AND Bookings.HairdresserID=? AND date(Bookings.[Date])>=date('now') AND date(Bookings.[Date])<=date('now','+1 day') ORDER BY Bookings.[Date],Bookings.Slot",(hid,))
        elif mode=="all":
...
def add_order_popup():
    ...
    def save():
        ...
        dt_str = dte.get().strip()
        try:
            odt = datetime.strptime(dt_str, "%Y-%m-%d %H:%M")
        except ValueError:
            errs.append(f"'{dt_str}' is not a valid date and time. Use the format YYYY-MM-DD HH:MM (e.g. 2025-06-15 14:30).")
            odt = None
        ...
        merged = {}
        for idx, ln in enumerate(order_lines, start=1):
            sel = ln["pb"].get().strip()
            if sel not in pm:
                errs.append(f"Row {idx}: please select a valid product from the dropdown.")
                continue
            info = pm[sel]
            pid2 = info["ProductID"]; stk = info["StockRemaining"]; pname = info["ProductName"]
            qty_str = ln["qe"].get().strip()
            if not qty_str:
                errs.append(f"Row {idx} ({pname}): quantity is required.")
                continue
            try:
                q = int(qty_str)
            except ValueError:
                errs.append(f"Row {idx} ({pname}): '{qty_str}' is not a valid quantity. Enter a whole number (e.g. 2).")
                continue
            if q <= 0:
                errs.append(f"Row {idx} ({pname}): quantity must be at least 1.")
                continue
            if stk <= 0:
                errs.append(f"Row {idx}: {pname} is out of stock and cannot be added to this order.")
                continue
            merged[pid2] = merged.get(pid2, {"name": pname, "price": info["Price"], "stock": stk, "qty": 0})
            merged[pid2]["qty"] += q
        ...
        for info in merged.values():
            if info["qty"] > info["stock"]:
                errs.append(
                    f"{info['name']}: you requested {info['qty']} but only "
                    f"{info['stock']} {'unit is' if info['stock']==1 else 'units are'} available."
                )
        ...
        # ── Save ─────────────────────────────────────────────────
        try:
            total = sum(i["qty"] * i["price"] for i in merged.values())
            cursor.execute(
                "INSERT INTO Orders(ClientID,OrderDate,PaymentMethod,TotalAmount) VALUES(?,?,?,?)",
                (int(cm[cname]), odt, pay, float(total))
            )
            conn.commit()
            cursor.execute("SELECT MAX(OrderID) FROM Orders")
            new_oid = safe_int(cursor.fetchone()[0])
            for pid2, info in merged.items():
                cursor.execute(
                    "INSERT INTO OrderItems(OrderID,ProductID,Quantity,PriceAtSale) VALUES(?,?,?,?)",
                    (new_oid, pid2, info["qty"], info["price"])
                )
                cursor.execute(
                    "UPDATE Products SET StockRemaining=StockRemaining-? WHERE ProductID=?",
                    (info["qty"], pid2)
                )
            conn.commit()
            done = CTkFrame(app, fg_color="#000000"); done.place(relx=0, rely=0, relwidth=1, relheight=1); done.lift()
            dp = CTkFrame(done, fg_color=C_SURFACE, corner_radius=18, width=340, height=160)
            dp.place(relx=0.5, rely=0.5, anchor="center")
            CTkLabel(dp, text="✓  Transaction Complete", font=FONT_SECTION, text_color=C_SUCCESS).pack(pady=(28, 6))
            CTkLabel(dp, text=f"Order #{new_oid} saved  •  Total Rs {total:.2f}", font=FONT_BODY, text_color=C_MUTED).pack()
            ...
            err.configure(text="The order could not be saved. Please check all fields and try again.")
...
def add_expense_popup():
    ...
    def save():
        ...
        cat     = cb.get().strip()
        title   = de.get().strip()
        amt_str = ae.get().strip()
        dt      = dte.get().strip()

        if not cat or cat not in EXPENSE_CATS:
            errs.append("Please select a category from the dropdown.")
        if not title:
            errs.append("Expense title is required.")
        if not amt_str:
            errs.append("Amount is required.")
        else:
            ok, amt_val = _vld_pos_float(amt_str)
            if not ok:
                errs.append(f"'{amt_str}' is not a valid amount. Enter a positive number in Rs (e.g. 1500.00).")
        if not dt:
            errs.append("Date is required.")
        else:
            ok2, _ = _vld_date(dt)
            if not ok2:
                errs.append(f"'{dt}' is not a valid date. Use YYYY-MM-DD (e.g. 2025-06-15).")
        if _show_errors(err, errs): return
        amt_val = float(amt_str)
        try:
            cursor.execute(
                "INSERT INTO Expenses (Title, Amount, Category, DateTime, Notes) VALUES (?, ?, ?, ?, ?)",
                (title, amt_val, cat, dt, "")
            )
            conn.commit()
            overlay.destroy()
            refresh_financials_page()
        except Exception:
            err.configure(text="The expense could not be saved. Please try again.")
...
def edit_expense_popup(eid, table=None):
    ...
    try:
        ...
        err.configure(text="Could not load the expense record. Please try again."); return
    if not ex:
        err.configure(text="Expense not found. It may have been deleted."); return
    ...
    def delete():
        try:
            ...
            err.configure(text="This expense could not be deleted. Please try again.")
            return
        overlay.destroy()
        refresh_financials_page()
...
_rpt_to_var      = None
_rpt_output      = None
_rpt_active_key  = [None]
_rpt_active_fig  = [None]
_rpt_active_data = [None]
_rpt_active_hdrs = [None]
_rpt_active_title= [""]

# ── Date helpers ──────────────────────────────────────────────────
def _rpt_parse_dates():
    try:
        s = datetime.strptime(_rpt_from_var.get().strip(), "%Y-%m-%d").date()
        e = datetime.strptime(_rpt_to_var.get().strip(), "%Y-%m-%d").date()
        return (None, None) if s > e else (s, e)
    except:
        return None, None

def _rpt_quick_date(mode):
    ...
    if mode == "today":
        ...
        s = datetime(2000, 1, 1).date(); e = today
    _rpt_from_var.set(s.strftime("%Y-%m-%d"))
    _rpt_to_var.set(e.strftime("%Y-%m-%d"))

# ── Export ────────────────────────────────────────────────────────
def _rpt_export_excel():
    title   = _rpt_active_title[0]
    headers = _rpt_active_hdrs[0]
    rows    = _rpt_active_data[0]
    if not headers or rows is None: return
    path = filedialog.asksaveasfilename(
    ...
    try:
        ...
        ws["A1"] = title
        ws["A1"].font = Font(bold=True, size=14, color="C9A84C")
...
def _rpt_export_pdf():
    ...
    try:
        with PdfPages(path) as pdf:
            # Title page
            ft = plt.figure(figsize=(11.7, 2.8))
            ft.patch.set_facecolor("#1a1a1a")
            ax = ft.add_subplot(111); ax.axis("off"); ax.set_facecolor("#1a1a1a")
            ax.text(0.5, 0.65, title, transform=ax.transAxes, fontsize=22,
                    color="#c9a84c", ha="center", va="center", weight="bold")
            ax.text(0.5, 0.25, f"Generated on {datetime.now().strftime('%d %B %Y at %H:%M')}",
                    transform=ax.transAxes, fontsize=11, color="#888888", ha="center")
            plt.tight_layout(pad=0.5)
            pdf.savefig(ft, bbox_inches="tight"); plt.close(ft)
            # Chart
            if fig_ref:
            ...
            if rows and headers:
                chunk_size = 28
                for cs in range(0, len(rows), chunk_size):
                    chunk = rows[cs:cs+chunk_size]
                    fh = max(3.5, len(chunk)*0.38+1.5)
                    fp = plt.figure(figsize=(11.7, fh))
                    fp.patch.set_facecolor("#1a1a1a")
                    axp = fp.add_subplot(111); axp.axis("off"); axp.set_facecolor("#1a1a1a")
                    cell_text = [[str(v) if v is not None else "—" for v in r] for r in chunk]
                    tbl = axp.table(cellText=cell_text, colLabels=list(headers),
                                    cellLoc="center", loc="center")
                    tbl.auto_set_font_size(False); tbl.set_fontsize(8); tbl.scale(1, 1.5)
                    for j in range(len(headers)):
                        tbl[0,j].set_facecolor("#c9a84c")
                        tbl[0,j].set_text_props(color="#1a1a1a", weight="bold")
                    for i in range(len(chunk)):
                        bg = "#2c2c2c" if i%2==0 else "#1e1e1e"
                        for j in range(len(headers)):
                            tbl[i+1,j].set_facecolor(bg)
                            tbl[i+1,j].set_text_props(color="#f0f0f0")
                    plt.tight_layout(pad=0.3)
                    pdf.savefig(fp, bbox_inches="tight"); plt.close(fp)
    except Exception as e:
        print("PDF export error:", e)

# ── Shared UI helpers ─────────────────────────────────────────────
def _rpt_header(parent, title, subtitle=""):
    _rpt_active_title[0] = title
    row = CTkFrame(parent, fg_color="transparent")
    row.pack(fill="x", padx=PAD, pady=(PAD, 4))
    CTkLabel(row, text=title, font=FONT_TITLE, text_color=C_TEXT).pack(side="left")
    if subtitle:
        CTkLabel(row, text=subtitle, font=FONT_SMALL,
                 text_color=C_MUTED).pack(side="left", padx=(14,0), pady=(6,0))

def _rpt_subtitle(parent, text):
    CTkLabel(parent, text=text, font=FONT_SMALL, text_color=C_MUTED,
             justify="left").pack(anchor="w", padx=PAD, pady=(0,8))

def _rpt_export_bar(parent):
    bar = CTkFrame(parent, fg_color="transparent")
    bar.pack(fill="x", padx=PAD, pady=(4, PAD))
    _gold_btn(bar, "⬇  Export Excel", _rpt_export_excel, width=155).pack(side="left", padx=(0,8))
    _ghost_btn(bar, "⬇  Export PDF",  _rpt_export_pdf,  width=140).pack(side="left")

def _rpt_no_data(parent):
    CTkLabel(parent, text="No data found for the selected date range.",
             font=FONT_BODY, text_color=C_MUTED).pack(pady=40)

def _rpt_embed_chart(parent, fig):
    _rpt_active_fig[0] = fig
    card = CTkFrame(parent, fg_color=C_SURFACE, corner_radius=RADIUS)
    card.pack(fill="x", padx=PAD, pady=(0,8))
    embed_chart(fig, card)

def _rpt_table(parent, headers, rows, col_weights=None):
    _rpt_active_hdrs[0] = headers
    _rpt_active_data[0] = rows
    card = CTkFrame(parent, fg_color=C_SURFACE, corner_radius=RADIUS)
    card.pack(fill="x", padx=PAD, pady=(0,8))
    h = min(40 + len(rows)*38, 340)
    wrap = CTkScrollableFrame(card, fg_color="transparent",
                              height=h, scrollbar_button_color=C_BORDER)
    wrap.pack(fill="x", padx=8, pady=8)
    for c in range(len(headers)):
        wrap.grid_columnconfigure(c, weight=(col_weights[c] if col_weights else 1))
    render_table_headers(wrap, list(headers))
    if not rows:
        empty_state(wrap, "No records.", len(headers)); return
    for i, row in enumerate(rows):
        for j, val in enumerate(row):
            table_cell(wrap, str(val) if val is not None else "—", i, j)

# ── DB query functions ────────────────────────────────────────────
def _rptq_revenue_time(start, end):
    try:
        cursor.execute("""
            SELECT CStr([Date]) AS d, SUM(Services.Price) AS rev, COUNT(*) AS bks
            FROM Bookings INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY CStr([Date]), [Date] ORDER BY [Date]
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq revenue_time:", e); return []

def _rptq_revenue_by_service(start, end):
    try:
        cursor.execute("""
            SELECT Services.ServiceName, COUNT(*) AS bks, SUM(Services.Price) AS rev
            FROM Bookings INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Services.ServiceName ORDER BY SUM(Services.Price) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq revenue_by_service:", e); return []

def _rptq_revenue_by_hairdresser(start, end):
    try:
        cursor.execute("""
            SELECT Hairdressers.HairdresserName, COUNT(*) AS bks, SUM(Services.Price) AS rev
            FROM (Bookings INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID)
            INNER JOIN Hairdressers ON Bookings.HairdresserID=Hairdressers.HairdresserID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Hairdressers.HairdresserName ORDER BY SUM(Services.Price) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq revenue_by_hairdresser:", e); return []

def _rptq_pl_monthly(start, end):
    try:
        cursor.execute("""
            SELECT Format([Date],'YYYY-MM') AS mo, SUM(Services.Price) AS rev
            FROM Bookings INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Format([Date],'YYYY-MM') ORDER BY Format([Date],'YYYY-MM')
        """, (start, end))
        rev_map = {r[0]: safe_float(r[1]) for r in cursor.fetchall()}
    except: rev_map = {}
    try:
        cursor.execute("""
            SELECT Format([DateTime],'YYYY-MM') AS mo, SUM(Amount) AS costs
            FROM Expenses WHERE [DateTime]>=? AND [DateTime]<=?
            GROUP BY Format([DateTime],'YYYY-MM') ORDER BY Format([DateTime],'YYYY-MM')
        """, (start, end))
        cost_map = {r[0]: safe_float(r[1]) for r in cursor.fetchall()}
    except: cost_map = {}
    months = sorted(set(list(rev_map)+list(cost_map)))
    return [(m, rev_map.get(m,0.0), cost_map.get(m,0.0)) for m in months]

def _rptq_client_bookings(start, end):
    try:
        cursor.execute("""
            SELECT Bookings.ClientID, Format([Date],'YYYY-MM') AS mo
            FROM Bookings WHERE [Date]>=? AND [Date]<=? AND Canceled=False
            GROUP BY Bookings.ClientID, Format([Date],'YYYY-MM')
            ORDER BY Format([Date],'YYYY-MM')
        """, (start, end)); in_range = cursor.fetchall()
    except: in_range = []
    try:
        cursor.execute("""
            SELECT ClientID, Format(Min([Date]),'YYYY-MM') AS first_mo
            FROM Bookings WHERE Canceled=False GROUP BY ClientID
        """)
        first_map = {r[0]: r[1] for r in cursor.fetchall()}
    except: first_map = {}
    return in_range, first_map

def _rptq_top_clients(start, end):
    try:
        cursor.execute("""
            SELECT Clients.ClientName, COUNT(*) AS visits, SUM(Services.Price) AS spend
            FROM (Bookings INNER JOIN Clients ON Bookings.ClientID=Clients.ClientID)
            INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Clients.ClientName ORDER BY SUM(Services.Price) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq top_clients:", e); return []

def _rptq_busiest_days(start, end):
    try:
        cursor.execute("""
            SELECT Weekday([Date]) AS wd, Format([Date],'dddd') AS dn, COUNT(*) AS bks
            FROM Bookings WHERE [Date]>=? AND [Date]<=? AND Canceled=False
            GROUP BY Weekday([Date]), Format([Date],'dddd')
            ORDER BY Weekday([Date])
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq busiest_days:", e); return []

def _rptq_busiest_slots(start, end):
    try:
        cursor.execute("""
            SELECT SlotData.Timings, COUNT(*) AS bks, SUM(Services.Price) AS rev
            FROM (Bookings INNER JOIN SlotData ON Bookings.Slot=SlotData.ID)
            INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=? AND Bookings.Canceled=False
            GROUP BY SlotData.Timings, SlotData.ID ORDER BY COUNT(*) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq busiest_slots:", e); return []

def _rptq_hairdresser_perf(start, end):
    try:
        cursor.execute("""
            SELECT Hairdressers.HairdresserName,
                   COUNT(*) AS total,
                   SUM(IIf(Bookings.IsCompleted=True AND Bookings.Canceled=False,1,0)) AS comp,
                   SUM(IIf(Bookings.Canceled=True,1,0)) AS cncl,
                   SUM(IIf(Bookings.IsCompleted=True AND Bookings.Canceled=False,Services.Price,0)) AS rev
            FROM (Bookings INNER JOIN Hairdressers ON Bookings.HairdresserID=Hairdressers.HairdresserID)
            INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
            GROUP BY Hairdressers.HairdresserName
            ORDER BY SUM(IIf(Bookings.IsCompleted=True AND Bookings.Canceled=False,Services.Price,0)) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq hairdresser_perf:", e); return []

def _rptq_popular_services(start, end):
    try:
        cursor.execute("""
            SELECT Services.ServiceName, COUNT(*) AS bks, SUM(Services.Price) AS rev
            FROM Bookings INNER JOIN Services ON Bookings.ServiceID=Services.ServiceID
            ...
            GROUP BY Services.ServiceName ORDER BY COUNT(*) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq popular_services:", e); return []

def _rptq_cancellations(start, end):
    try:
        cursor.execute("""
            SELECT Format([Date],'YYYY-MM') AS mo, COUNT(*) AS total,
                   SUM(IIf(Canceled=True,1,0)) AS cncl
            FROM Bookings WHERE [Date]>=? AND [Date]<=?
            GROUP BY Format([Date],'YYYY-MM') ORDER BY Format([Date],'YYYY-MM')
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq cancellations:", e); return []

def _rptq_product_sales(start, end):
    try:
        cursor.execute("""
            SELECT Products.ProductName,
                   SUM(OrderItems.Quantity) AS units,
                   SUM(OrderItems.Quantity*OrderItems.PriceAtSale) AS rev,
                   Products.StockRemaining AS stock
            ...
            GROUP BY Products.ProductName, Products.StockRemaining
            ORDER BY SUM(OrderItems.Quantity*OrderItems.PriceAtSale) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq product_sales:", e); return []

def _rptq_churn(days=45):
    try:
        cursor.execute("""
            SELECT Clients.ClientName, MAX(Bookings.[Date]) AS last_v, COUNT(*) AS visits
            FROM Clients INNER JOIN Bookings ON Clients.ClientID=Bookings.ClientID
            WHERE Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Clients.ClientName ORDER BY MAX(Bookings.[Date]) ASC
        """)
        rows = cursor.fetchall()
        result = []
        today = datetime.now().date()
        for name, last, visits in rows:
            if last:
                ldate = last.date() if hasattr(last,'date') else datetime.strptime(str(last)[:10],"%Y-%m-%d").date()
                ds = (today - ldate).days
                if ds >= days:
                    risk = "🔴 High Risk" if ds >= 90 else "🟡 At Risk"
                    result.append((name, ldate.strftime("%Y-%m-%d"), ds, safe_int(visits), risk))
        return result
    except Exception as e: print("rptq churn:", e); return []

def _rptq_peak_slot(start, end):
    try:
        cursor.execute("""
            SELECT SlotData.Timings, COUNT(*) AS bks, SUM(Services.Price) AS rev
            FROM (Bookings INNER JOIN SlotData ON Bookings.Slot=SlotData.ID)
            ...
            WHERE Bookings.[Date]>=? AND Bookings.[Date]<=?
              AND Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY SlotData.Timings, SlotData.ID ORDER BY SUM(Services.Price) DESC
        """, (start, end)); return cursor.fetchall()
    except Exception as e: print("rptq peak_slot:", e); return []

def _rptq_service_loyalty():
    try:
        cursor.execute("""
            SELECT Bookings.ServiceID, Bookings.ClientID, COUNT(*) AS times
            FROM Bookings WHERE Canceled=False
            GROUP BY Bookings.ServiceID, Bookings.ClientID
        """)
        rows = cursor.fetchall()
        from collections import defaultdict
        total_c  = defaultdict(set)
        repeat_c = defaultdict(set)
        for sid, cid, times in rows:
            total_c[sid].add(cid)
            if times > 1: repeat_c[sid].add(cid)
        cursor.execute("SELECT ServiceID, ServiceName FROM Services")
        svc_map = {r[0]: r[1] for r in cursor.fetchall()}
        result = []
        for sid, name in svc_map.items():
            t = len(total_c.get(sid,set())); r = len(repeat_c.get(sid,set()))
            if t > 0: result.append((name, t, r, round(r/t*100,1)))
        result.sort(key=lambda x: x[3], reverse=True)
        return result
    except Exception as e: print("rptq service_loyalty:", e); return []

def _rptq_client_segments():
    try:
        cursor.execute("""
            SELECT Clients.ClientName, Clients.ClientID, COUNT(*) AS visits
            FROM Clients INNER JOIN Bookings ON Clients.ClientID=Bookings.ClientID
            WHERE Bookings.IsCompleted=True AND Bookings.Canceled=False
            GROUP BY Clients.ClientName, Clients.ClientID ORDER BY COUNT(*) DESC
        """)
        rows = cursor.fetchall()
        result = []
        for name, cid, visits in rows:
            v = safe_int(visits)
            seg = "👑 Champion" if v>=10 else "⭐ Loyal" if v>=5 else "🌱 Growing" if v>=2 else "🆕 New"
            result.append((name, v, seg))
        return result
    except Exception as e: print("rptq client_segments:", e); return []

# ── Report renderers ──────────────────────────────────────────────

def _rpt_revenue_time(parent, start, end):
    _rpt_header(parent, "Revenue Over Time", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Daily service revenue for completed bookings in the selected period.")
    rows = _rptq_revenue_time(start, end)
    if not rows: _rpt_no_data(parent); return
    dates = [r[0] for r in rows]; revs = [safe_float(r[1]) for r in rows]; bks = [safe_int(r[2]) for r in rows]
    fig, ax = _chart_fig(9, 3.2)
    ax.bar(range(len(dates)), revs, color=C_GOLD, alpha=0.9)
    step = max(1, len(dates)//12)
    ax.set_xticks(range(len(dates)))
    ax.set_xticklabels([d if i%step==0 else "" for i,d in enumerate(dates)],
                       color=C_MUTED, fontsize=8, rotation=30, ha="right")
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Daily Revenue (Completed Services)", color=C_TEXT, fontsize=12, pad=8)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Date","Revenue","Bookings"),
               [(dates[i], f"Rs {revs[i]:,.2f}", bks[i]) for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_revenue_service(parent, start, end):
    _rpt_header(parent, "Revenue by Service", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Which services are generating the most income.")
    rows = _rptq_revenue_by_service(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; revs=[safe_float(r[2]) for r in rows]; bks=[safe_int(r[1]) for r in rows]
    fig, ax = _chart_fig(9, max(3, len(names)*0.5))
    ax.barh(range(len(names)), revs, color=C_GOLD, alpha=0.9)
    ax.set_yticks(range(len(names))); ax.set_yticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="x", colors=C_MUTED)
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Revenue by Service (Completed)", color=C_TEXT, fontsize=12, pad=8)
    ax.invert_yaxis(); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    total = sum(revs)
    _rpt_table(parent, ("Service","Bookings","Revenue","% of Total"),
               [(names[i], bks[i], f"Rs {revs[i]:,.2f}",
                 f"{revs[i]/total*100:.1f}%" if total else "—") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_revenue_hairdresser(parent, start, end):
    _rpt_header(parent, "Revenue by Hairdresser", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Bookings and revenue side-by-side for each hairdresser.")
    rows = _rptq_revenue_by_hairdresser(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; bks=[safe_int(r[1]) for r in rows]; revs=[safe_float(r[2]) for r in rows]
    x=range(len(names)); w=0.38
    fig, ax = _chart_fig(9, 3.4); ax2=ax.twinx()
    ax.bar([i-w/2 for i in x], bks,  width=w, color=C_MUTED,  label="Bookings", alpha=0.8)
    ax2.bar([i+w/2 for i in x], revs, width=w, color=C_GOLD, label="Revenue",  alpha=0.9)
    ax.set_xticks(list(x)); ax.set_xticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="y", colors=C_MUTED); ax2.tick_params(axis="y", colors=C_GOLD)
    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Hairdresser Bookings & Revenue", color=C_TEXT, fontsize=12, pad=8)
    l1,lb1=ax.get_legend_handles_labels(); l2,lb2=ax2.get_legend_handles_labels()
    ax.legend(l1+l2, lb1+lb2, facecolor=C_SURFACE2, labelcolor=C_TEXT, fontsize=8)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Hairdresser","Bookings","Revenue","Avg / Booking"),
               [(names[i], bks[i], f"Rs {revs[i]:,.2f}",
                 f"Rs {revs[i]/bks[i]:.2f}" if bks[i] else "—") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_pl_summary(parent, start, end):
    _rpt_header(parent, "P&L Summary", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Monthly revenue vs costs vs net profit. Costs pulled from Expenses log.")
    rows = _rptq_pl_monthly(start, end)
    if not rows: _rpt_no_data(parent); return
    months=[r[0] for r in rows]; revs=[safe_float(r[1]) for r in rows]
    costs=[safe_float(r[2]) for r in rows]; nets=[r-c for r,c in zip(revs,costs)]
    x=range(len(months)); w=0.28
    fig, ax = _chart_fig(9, 3.4)
    ax.bar([i-w for i in x], revs,  width=w, color=C_GOLD,   label="Revenue",    alpha=0.9)
    ax.bar([i   for i in x], costs, width=w, color=C_DANGER, label="Costs",      alpha=0.9)
    ax.bar([i+w for i in x], nets,  width=w, color=[C_SUCCESS if n>=0 else C_DANGER for n in nets],
           label="Net Profit", alpha=0.9)
    ax.set_xticks(list(x)); ax.set_xticklabels(months, color=C_MUTED, fontsize=9)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Monthly Revenue  vs  Costs  vs  Net Profit", color=C_TEXT, fontsize=12, pad=8)
    ax.legend(facecolor=C_SURFACE2, labelcolor=C_TEXT, fontsize=8)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Month","Revenue","Costs","Net Profit"),
               [(months[i], f"Rs {revs[i]:,.2f}", f"Rs {costs[i]:,.2f}", f"Rs {nets[i]:,.2f}")
                for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_client_retention(parent, start, end):
    _rpt_header(parent, "Client Retention", f"{start}  →  {end}")
    _rpt_subtitle(parent, "New clients (first ever visit) vs returning clients, per month.")
    in_range, first_map = _rptq_client_bookings(start, end)
    if not in_range: _rpt_no_data(parent); return
    from collections import defaultdict
    new_pm=defaultdict(int); ret_pm=defaultdict(int)
    for cid, mo in in_range:
        (new_pm if first_map.get(cid)==mo else ret_pm)[mo] += 1
    months=sorted(set(list(new_pm)+list(ret_pm)))
    nv=[new_pm[m] for m in months]; rv=[ret_pm[m] for m in months]
    fig, ax = _chart_fig(9, 3.2)
    ax.bar(range(len(months)), rv, label="Returning", color=C_GOLD,    alpha=0.9)
    ax.bar(range(len(months)), nv, bottom=rv, label="New", color=C_SUCCESS, alpha=0.9)
    ax.set_xticks(range(len(months))); ax.set_xticklabels(months, color=C_MUTED, fontsize=9)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.set_title("New vs Returning Clients per Month", color=C_TEXT, fontsize=12, pad=8)
    ax.legend(facecolor=C_SURFACE2, labelcolor=C_TEXT, fontsize=9)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Month","New Clients","Returning","Total"),
               [(months[i], nv[i], rv[i], nv[i]+rv[i]) for i in range(len(months))])
    _rpt_export_bar(parent)

def _rpt_top_clients(parent, start, end):
    _rpt_header(parent, "Top Clients by Spend", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Your most valuable clients ranked by total service spend. Top 15 shown.")
    rows = _rptq_top_clients(start, end)
    if not rows: _rpt_no_data(parent); return
    top=rows[:15]; names=[r[0] for r in top]; spends=[safe_float(r[2]) for r in top]; visits=[safe_int(r[1]) for r in top]
    fig, ax = _chart_fig(9, max(3, len(names)*0.46))
    ax.barh(range(len(names)), spends, color=C_GOLD, alpha=0.9)
    ax.set_yticks(range(len(names))); ax.set_yticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="x", colors=C_MUTED)
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Top Clients — Total Spend", color=C_TEXT, fontsize=12, pad=8)
    ax.invert_yaxis(); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Rank","Client","Visits","Total Spend","Avg / Visit"),
               [(f"#{i+1}", names[i], visits[i], f"Rs {spends[i]:,.2f}",
                 f"Rs {spends[i]/visits[i]:.2f}" if visits[i] else "—")
                for i in range(len(top))])
    _rpt_export_bar(parent)

def _rpt_busiest_days(parent, start, end):
    _rpt_header(parent, "Busiest Days of the Week", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Which weekdays receive the most bookings. Helps with staffing decisions.")
    rows = _rptq_busiest_days(start, end)
    if not rows: _rpt_no_data(parent); return
    days=[r[1] for r in rows]; bks=[safe_int(r[2]) for r in rows]
    fig, ax = _chart_fig(9, 3.2)
    peak=max(bks); colors=[C_GOLD if b==peak else C_SURFACE2 for b in bks]
    bars=ax.bar(range(len(days)), bks, color=colors, alpha=0.9)
    ax.set_xticks(range(len(days))); ax.set_xticklabels(days, color=C_TEXT, fontsize=10)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.set_title("Bookings by Day of Week", color=C_TEXT, fontsize=12, pad=8)
    for bar,val in zip(bars,bks):
        ax.text(bar.get_x()+bar.get_width()/2, bar.get_height()+0.3,
                str(val), ha="center", color=C_MUTED, fontsize=9)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Day","Total Bookings"), [(days[i], bks[i]) for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_busiest_slots(parent, start, end):
    _rpt_header(parent, "Busiest Time Slots", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Which appointment slots fill up fastest.")
    rows = _rptq_busiest_slots(start, end)
    if not rows: _rpt_no_data(parent); return
    slots=[str(r[0]) for r in rows]; bks=[safe_int(r[1]) for r in rows]; revs=[safe_float(r[2]) for r in rows]
    fig, ax = _chart_fig(9, max(3, len(slots)*0.46))
    ax.barh(range(len(slots)), bks, color=C_GOLD, alpha=0.9)
    ax.set_yticks(range(len(slots))); ax.set_yticklabels(slots, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="x", colors=C_MUTED)
    ax.set_title("Bookings per Time Slot", color=C_TEXT, fontsize=12, pad=8)
    ax.invert_yaxis(); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Time Slot","Bookings","Revenue"),
               [(slots[i], bks[i], f"Rs {revs[i]:,.2f}") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_hairdresser_perf(parent, start, end):
    _rpt_header(parent, "Hairdresser Performance", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Completed vs canceled bookings and total revenue per hairdresser.")
    rows = _rptq_hairdresser_perf(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; comp=[safe_int(r[2]) for r in rows]
    cncl=[safe_int(r[3]) for r in rows]; revs=[safe_float(r[4]) for r in rows]
    x=range(len(names)); w=0.3
    fig, ax = _chart_fig(9, 3.4)
    ax.bar([i-w/2 for i in x], comp, width=w, color=C_SUCCESS, label="Completed", alpha=0.9)
    ax.bar([i+w/2 for i in x], cncl, width=w, color=C_DANGER,  label="Canceled",  alpha=0.9)
    ax.set_xticks(list(x)); ax.set_xticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.set_title("Completed vs Canceled per Hairdresser", color=C_TEXT, fontsize=12, pad=8)
    ax.legend(facecolor=C_SURFACE2, labelcolor=C_TEXT, fontsize=8)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Hairdresser","Total","Completed","Canceled","Revenue","Avg / Booking"),
               [(names[i], safe_int(rows[i][1]), comp[i], cncl[i], f"Rs {revs[i]:,.2f}",
                 f"Rs {revs[i]/comp[i]:.2f}" if comp[i] else "—") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_popular_services(parent, start, end):
    _rpt_header(parent, "Most Popular Services", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Services ranked by total bookings including walk-ins.")
    rows = _rptq_popular_services(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; bks=[safe_int(r[1]) for r in rows]; revs=[safe_float(r[2]) for r in rows]
    total_bks=sum(bks)
    COLORS=[C_GOLD,C_SUCCESS,C_WARNING,"#9b7fe8",C_MUTED,C_DANGER]
    fig, ax = _chart_fig(6, 4)
    ax.pie(bks, labels=names, colors=[COLORS[i%len(COLORS)] for i in range(len(names))],
           autopct="%1.1f%%", startangle=90, pctdistance=0.75,
           wedgeprops=dict(width=0.5), textprops={"color":C_MUTED,"fontsize":9})
    ax.set_title("Service Popularity (Booking Count)", color=C_TEXT, fontsize=12, pad=8)
    plt.tight_layout(pad=0.8)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Service","Bookings","% Share","Revenue"),
               [(names[i], bks[i],
                 f"{bks[i]/total_bks*100:.1f}%" if total_bks else "—",
                 f"Rs {revs[i]:,.2f}") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_cancellations(parent, start, end):
    _rpt_header(parent, "Cancellation Rate Analysis", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Monthly cancellation trend. High rates may indicate scheduling or service issues.")
    rows = _rptq_cancellations(start, end)
    if not rows: _rpt_no_data(parent); return
    months=[r[0] for r in rows]; totals=[safe_int(r[1]) for r in rows]
    cncld=[safe_int(r[2]) for r in rows]
    rates=[round(c/t*100,1) if t else 0 for c,t in zip(cncld,totals)]
    fig, ax = _chart_fig(9, 3.2)
    ax.plot(range(len(months)), rates, color=C_DANGER, marker="o", lw=2, ms=6)
    ax.fill_between(range(len(months)), rates, alpha=0.15, color=C_DANGER)
    ax.set_xticks(range(len(months))); ax.set_xticklabels(months, color=C_MUTED, fontsize=9)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"{v:.0f}%"))
    ax.set_title("Monthly Cancellation Rate", color=C_TEXT, fontsize=12, pad=8)
    ax.set_ylim(0, max(rates+[5])+5); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Month","Total Bookings","Canceled","Cancel Rate"),
               [(months[i], totals[i], cncld[i], f"{rates[i]}%") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_product_sales(parent, start, end):
    _rpt_header(parent, "Product Sales & Stock", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Units sold and revenue per product. Items with ≤5 units flagged for restock.")
    rows = _rptq_product_sales(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; units=[safe_int(r[1]) for r in rows]
    revs=[safe_float(r[2]) for r in rows]; stock=[safe_int(r[3]) for r in rows]
    fig, ax = _chart_fig(9, max(3, len(names)*0.46))
    ax.barh(range(len(names)), revs, color=C_SUCCESS, alpha=0.9)
    ax.set_yticks(range(len(names))); ax.set_yticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="x", colors=C_MUTED)
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Product Revenue in Period", color=C_TEXT, fontsize=12, pad=8)
    ax.invert_yaxis(); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Product","Units Sold","Revenue","Stock Left","Status"),
               [(names[i], units[i], f"Rs {revs[i]:,.2f}", stock[i],
                 "⚠ Restock" if stock[i]<=5 else "✓ OK") for i in range(len(rows))])
    _rpt_export_bar(parent)

# ── Smart Insight renderers ───────────────────────────────────────

def _rpt_churn(parent, start, end):
    _rpt_header(parent, "Client Churn Risk")
    _rpt_subtitle(parent, "🟡 At Risk = 45–89 days since last visit.  🔴 High Risk = 90+ days.  Use this as your call list.")
    rows = _rptq_churn(days=45)
    if not rows:
        CTkLabel(parent, text="🎉  Everyone has visited recently — no at-risk clients!",
                 font=FONT_BODY, text_color=C_SUCCESS).pack(pady=40)
        _rpt_active_hdrs[0] = None; _rpt_active_data[0] = []
        return
    high=[r for r in rows if r[4].startswith("🔴")]; mid=[r for r in rows if r[4].startswith("🟡")]
    if high or mid:
        fig, ax = _chart_fig(5, 3.2)
        counts=[len(mid),len(high)]; labels=[f"At Risk\n({len(mid)})",f"High Risk\n({len(high)})"]
        ax.pie([c for c in counts if c>0],
               labels=[l for c,l in zip(counts,labels) if c>0],
               colors=[C_WARNING,C_DANGER][:sum(1 for c in counts if c>0)],
               autopct="%1.0f%%", startangle=90, pctdistance=0.75,
               wedgeprops=dict(width=0.5), textprops={"color":C_MUTED,"fontsize":10})
        ax.set_title("Churn Risk Breakdown", color=C_TEXT, fontsize=12, pad=8)
        plt.tight_layout(pad=0.8)
        _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Client","Last Visit","Days Since Visit","Total Visits","Risk Level"), rows)
    _rpt_export_bar(parent)

def _rpt_peak_slot(parent, start, end):
    _rpt_header(parent, "Peak Revenue Slot", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Not just the busiest slot — the one that earns the most. A quiet slot with premium services beats a busy cheap one.")
    rows = _rptq_peak_slot(start, end)
    if not rows: _rpt_no_data(parent); return
    slots=[str(r[0]) for r in rows]; bks=[safe_int(r[1]) for r in rows]; revs=[safe_float(r[2]) for r in rows]
    avg_v=[r/b if b else 0 for r,b in zip(revs,bks)]
    fig, (ax1,ax2) = plt.subplots(1,2,figsize=(9,3.4))
    fig.patch.set_facecolor(C_CHART_BG)
    for ax in (ax1,ax2):
        ax.set_facecolor(C_CHART_BG); ax.tick_params(colors=C_MUTED)
        for sp in ax.spines.values(): sp.set_edgecolor(C_BORDER)
    y=range(len(slots))
    ax1.barh(list(y), revs, color=C_GOLD, alpha=0.9)
    ax1.set_yticks(list(y)); ax1.set_yticklabels(slots, color=C_TEXT, fontsize=8)
    ax1.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax1.set_title("Total Revenue / Slot", color=C_TEXT, fontsize=10); ax1.invert_yaxis()
    ax2.barh(list(y), avg_v, color="#9b7fe8", alpha=0.9)
    ax2.set_yticks(list(y)); ax2.set_yticklabels(slots, color=C_TEXT, fontsize=8)
    ax2.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax2.set_title("Avg Revenue per Booking", color=C_TEXT, fontsize=10); ax2.invert_yaxis()
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Time Slot","Bookings","Total Revenue","Avg Value / Booking"),
               [(slots[i], bks[i], f"Rs {revs[i]:,.2f}", f"Rs {avg_v[i]:,.2f}")
                for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_efficiency(parent, start, end):
    _rpt_header(parent, "Hairdresser Efficiency", f"{start}  →  {end}")
    _rpt_subtitle(parent, "Revenue per completed booking. Reveals who books premium services vs who undercharges. Gold bar = top performer.")
    rows = _rptq_hairdresser_perf(start, end)
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; comp=[safe_int(r[2]) for r in rows]; revs=[safe_float(r[4]) for r in rows]
    eff=[r/c if c else 0 for r,c in zip(revs,comp)]
    data=sorted(zip(names,eff,comp,revs), key=lambda x:x[1], reverse=True)
    sn=[d[0] for d in data]; se=[d[1] for d in data]; sc=[d[2] for d in data]; sr=[d[3] for d in data]
    fig, ax = _chart_fig(9, 3.2)
    colors=[C_GOLD if i==0 else C_SURFACE2 for i in range(len(sn))]
    bars=ax.bar(range(len(sn)), se, color=colors, alpha=0.9)
    ax.set_xticks(range(len(sn))); ax.set_xticklabels(sn, color=C_TEXT, fontsize=10)
    ax.tick_params(axis="y", colors=C_MUTED)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"Rs {v:,.0f}"))
    ax.set_title("Avg Revenue per Completed Booking", color=C_TEXT, fontsize=12, pad=8)
    for bar,val in zip(bars,se):
        ax.text(bar.get_x()+bar.get_width()/2, bar.get_height()+2,
                f"Rs {val:,.0f}", ha="center", color=C_MUTED, fontsize=8)
    plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Hairdresser","Completed Bookings","Revenue","Avg / Booking"),
               [(sn[i], sc[i], f"Rs {sr[i]:,.2f}", f"Rs {se[i]:,.2f}") for i in range(len(data))])
    _rpt_export_bar(parent)

def _rpt_service_loyalty(parent, start, end):
    _rpt_header(parent, "Service Loyalty Index")
    _rpt_subtitle(parent, "All-time data: % of clients who booked the same service more than once. High % = that service creates habits.")
    rows = _rptq_service_loyalty()
    if not rows: _rpt_no_data(parent); return
    names=[r[0] for r in rows]; total=[r[1] for r in rows]; repeat=[r[2] for r in rows]; pcts=[r[3] for r in rows]
    fig, ax = _chart_fig(9, max(3, len(names)*0.5))
    colors=[C_GOLD if p>=50 else C_WARNING if p>=25 else C_MUTED for p in pcts]
    ax.barh(range(len(names)), pcts, color=colors, alpha=0.9)
    ax.set_yticks(range(len(names))); ax.set_yticklabels(names, color=C_TEXT, fontsize=9)
    ax.tick_params(axis="x", colors=C_MUTED)
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda v,_: f"{v:.0f}%"))
    ax.set_title("% of Clients Who Re-booked Same Service", color=C_TEXT, fontsize=12, pad=8)
    ax.invert_yaxis(); plt.tight_layout(pad=1.0)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Service","Unique Clients","Repeat Bookers","Loyalty %"),
               [(names[i], total[i], repeat[i], f"{pcts[i]}%") for i in range(len(rows))])
    _rpt_export_bar(parent)

def _rpt_client_segments(parent, start, end):
    _rpt_header(parent, "Client Lifetime Segments")
    _rpt_subtitle(parent, "All-time classification.  🆕 New = 1 visit  |  🌱 Growing = 2–4  |  ⭐ Loyal = 5–9  |  👑 Champion = 10+")
    rows = _rptq_client_segments()
    if not rows: _rpt_no_data(parent); return
    from collections import Counter
    seg_c=Counter(r[2] for r in rows)
    order=["🆕 New","🌱 Growing","⭐ Loyal","👑 Champion"]
    colors_s=[C_MUTED,C_SUCCESS,C_GOLD,"#9b7fe8"]
    labels=[s for s in order if s in seg_c]; counts=[seg_c[s] for s in labels]
    clrs=[colors_s[order.index(s)] for s in labels]
    fig, ax = _chart_fig(6, 4)
    ax.pie(counts, labels=[f"{l}\n({c})" for l,c in zip(labels,counts)],
           colors=clrs, autopct="%1.0f%%", startangle=90, pctdistance=0.75,
           wedgeprops=dict(width=0.5), textprops={"color":C_MUTED,"fontsize":9})
    ax.set_title("Client Loyalty Tier Distribution", color=C_TEXT, fontsize=12, pad=8)
    plt.tight_layout(pad=0.8)
    _rpt_embed_chart(parent, fig)
    _rpt_table(parent, ("Client","Total Visits","Tier"), rows)
    _rpt_export_bar(parent)

# ── Dispatcher ────────────────────────────────────────────────────
_REPORT_RENDERERS = {
    "revenue_time":        _rpt_revenue_time,
    "revenue_service":     _rpt_revenue_service,
    "revenue_hairdresser": _rpt_revenue_hairdresser,
    "pl_summary":          _rpt_pl_summary,
    "client_retention":    _rpt_client_retention,
    "top_clients":         _rpt_top_clients,
    "busiest_days":        _rpt_busiest_days,
    "busiest_slots":       _rpt_busiest_slots,
    "hairdresser_perf":    _rpt_hairdresser_perf,
    "popular_services":    _rpt_popular_services,
    "cancellations":       _rpt_cancellations,
    "product_sales":       _rpt_product_sales,
    "churn":               _rpt_churn,
    "peak_slot":           _rpt_peak_slot,
    "efficiency":          _rpt_efficiency,
    "service_loyalty":     _rpt_service_loyalty,
    "client_segments":     _rpt_client_segments,
}

def _run_report(key):
    _rpt_active_key[0]=key; _rpt_active_fig[0]=None
    _rpt_active_data[0]=None; _rpt_active_hdrs[0]=None
    _rpt_active_title[0]=key.replace("_"," ").title()
    start, end = _rpt_parse_dates()
    if start is None:
        clear_frame(_rpt_output)
        CTkLabel(_rpt_output, text="⚠  Invalid date range. Use YYYY-MM-DD format.",
                 font=FONT_BODY, text_color=C_DANGER).pack(pady=40)
        return
    clear_frame(_rpt_output)
    fn = _REPORT_RENDERERS.get(key)
    if fn: fn(_rpt_output, start, end)

# ── Main layout builder ───────────────────────────────────────────
def _build_reports(parent):
    global _rpt_from_var, _rpt_to_var, _rpt_output
    ...
    # ── LEFT PANEL ────────────────────────────────────────────────
    left = CTkFrame(parent, width=248, fg_color=C_SIDEBAR, corner_radius=0)
    left.grid(row=0, column=0, sticky="nsew"); left.grid_propagate(False)

    CTkLabel(left, text="📅  Date Range", font=FONT_BOLD,
             text_color=C_GOLD).pack(anchor="w", padx=14, pady=(16,6))

    today = datetime.now().date()
    _rpt_from_var = StringVar(value=today.replace(day=1).strftime("%Y-%m-%d"))
    _rpt_to_var   = StringVar(value=today.strftime("%Y-%m-%d"))

    dr = CTkFrame(left, fg_color="transparent"); dr.pack(fill="x", padx=14)
    for lbl, var, row_n in [("From", _rpt_from_var, 0), ("To", _rpt_to_var, 2)]:
        CTkLabel(dr, text=lbl, font=FONT_SMALL, text_color=C_MUTED).grid(
            row=row_n, column=0, sticky="w", pady=(0,2))
        CTkEntry(dr, textvariable=var, width=216, height=30,
                 fg_color=C_SURFACE, border_color=C_BORDER,
                 text_color=C_TEXT, font=FONT_SMALL).grid(
            row=row_n+1, column=0, pady=(0,6))

    for qrow, btns in [
        (CTkFrame(left, fg_color="transparent"),
         [("Today","today"),("This Week","week")]),
        (CTkFrame(left, fg_color="transparent"),
         [("This Month","month"),("This Year","year")]),
    ]:
        qrow.pack(fill="x", padx=14, pady=(0,2))
        for lbl,mode in btns:
            CTkButton(qrow, text=lbl, width=102, height=26, corner_radius=6,
                      fg_color=C_SURFACE2, hover_color=C_SURFACE,
                      text_color=C_TEXT_DIM, font=FONT_SMALL,
                      command=lambda m=mode: _rpt_quick_date(m)).pack(side="left", padx=2)
    CTkButton(left, text="All Time", width=216, height=26, corner_radius=6,
              fg_color=C_SURFACE2, hover_color=C_SURFACE,
              text_color=C_TEXT_DIM, font=FONT_SMALL,
              command=lambda: _rpt_quick_date("all")).pack(padx=14, pady=(0,10))

    CTkFrame(left, fg_color=C_BORDER, height=1).pack(fill="x", padx=14, pady=(0,8))

    # Report picker (scrollable)
    picker = CTkScrollableFrame(left, fg_color="transparent",
                                scrollbar_button_color=C_BORDER)
    picker.pack(fill="both", expand=True)

    def _cat(text):
        CTkLabel(picker, text=text, font=FONT_SMALL,
                 text_color=C_GOLD).pack(anchor="w", padx=14, pady=(10,3))

    def _rpt_btn(label, key):
        CTkButton(picker, text=f"  {label}", anchor="w",
                  width=224, height=34, corner_radius=8,
                  fg_color="transparent", hover_color=C_SURFACE,
                  text_color=C_TEXT_DIM, font=FONT_BODY,
                  command=lambda k=key: _run_report(k)).pack(padx=10, pady=2)

    _cat("💰  FINANCIAL")
    _rpt_btn("Revenue Over Time",        "revenue_time")
    _rpt_btn("Revenue by Service",       "revenue_service")
    _rpt_btn("Revenue by Hairdresser",   "revenue_hairdresser")
    _rpt_btn("P&L Summary",              "pl_summary")

    _cat("👥  MARKETING")
    _rpt_btn("Client Retention",         "client_retention")
    _rpt_btn("Top Clients by Spend",     "top_clients")

    _cat("⚡  OPERATIONS")
    _rpt_btn("Busiest Days",             "busiest_days")
    _rpt_btn("Busiest Time Slots",       "busiest_slots")
    _rpt_btn("Hairdresser Performance",  "hairdresser_perf")
    _rpt_btn("Most Popular Services",    "popular_services")
    _rpt_btn("Cancellation Rates",       "cancellations")
    _rpt_btn("Product Sales & Stock",    "product_sales")

    _cat("🧠  SMART INSIGHTS")
    _rpt_btn("Client Churn Risk",        "churn")
    _rpt_btn("Peak Revenue Slot",        "peak_slot")
    _rpt_btn("Hairdresser Efficiency",   "efficiency")
    _rpt_btn("Service Loyalty Index",    "service_loyalty")
    _rpt_btn("Client Lifetime Segments", "client_segments")

    # ── RIGHT PANEL ───────────────────────────────────────────────
    right_wrap = CTkFrame(parent, fg_color=C_BG, corner_radius=0)
    ...
    _rpt_output = CTkScrollableFrame(right_wrap, fg_color=C_BG,
                                     scrollbar_button_color=C_BORDER)
    _rpt_output.grid(row=0, column=0, sticky="nsew")

    # Welcome state
    CTkLabel(_rpt_output, text="📊", font=("Inter", 56)).pack(pady=(60,8))
    CTkLabel(_rpt_output, text="Select a report from the left panel",
             font=FONT_SECTION, text_color=C_TEXT).pack()
    CTkLabel(_rpt_output,
             text="Set your date range, then click any report.\nAll reports export to Excel and PDF.",
             font=FONT_BODY, text_color=C_MUTED, justify="center").pack(pady=(6,0))

def refresh_reports_page():
    clear_frame(reports_frame)
    _build_reports(reports_frame)
...
def show_content(tab):
    ...
    if tab=="Dashboard":
        ...
        services_frame.grid(row=0,column=0,sticky="nsew"); refresh_services_page()
    elif tab=="Schedule":
        ...
        reports_frame.grid(row=0,column=0,sticky="nsew"); refresh_reports_page()
